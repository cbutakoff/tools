...
void DensityFromGM(vtkPolyData* mesh, std::vector<double> scales);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cout << "-s start end step \t\t -gaussian scales" << std::endl;
        return -1;
    }
    ...
    double s_start = 0.1; //guassian scale
    double s_end = 0.3;
    double s_step = 0.1;
    ...
    for (double i = s_start; i < s_end; i += s_step)
        scales.push_back(i);

    DensityFromGM(mesh, scales);
...
void DensityFromGM(vtkPolyData* mesh, std::vector<double> scales) {
    vtkSmartPointer<vtkDoubleArray> vtk_density = vtkSmartPointer<vtkDoubleArray>::New();
    vtk_density->SetName("Density");
    vtk_density->SetNumberOfComponents(scales.size());
    vtk_density->SetNumberOfTuples(mesh->GetNumberOfPoints());

    //initialize storage
    std::vector<std::vector<double> > density;
    density.resize(mesh->GetNumberOfPoints());

    for (int i = 0; i < density.size(); i++)
    {
        density[i].resize(scales.size(),0.0);
    }

    
    //main loop
    for (int ncomponent = 0; ncomponent < scales.size(); ncomponent++) {

        
        const double s = scales[ncomponent];
        const double s2inv = 1 / (2 * s * s);
        std::cout<<"Scale "<<s<<std::endl;

        for (int i = 0; i < mesh->GetNumberOfPoints(); i++) {
            double pt[3];
            mesh->GetPoint(i, pt);
            double d_value = density[i][ncomponent];

            for (int j = 0; j < mesh->GetNumberOfPoints(); j++) {
                double pt1[3];
                mesh->GetPoint(j, pt1);
                const double pt0diff = pt1[0] - pt[0];
                const double pt1diff = pt1[1] - pt[1];
                const double pt2diff = pt1[2] - pt[2];
                const double d = pt0diff * pt0diff + pt1diff * pt1diff + pt2diff*pt2diff;
                const double w = exp(-d * s2inv);
                d_value += w;
            }

            density[i][ncomponent] = d_value;

            if (i % 100 == 0)
                std::cout << "Vertex: " << i << "/" << mesh->GetNumberOfPoints() << "\r";
        }
        std::cout << std::endl;
    }
    ...
    //copy the values
    for(int i=0; i<mesh->GetNumberOfPoints(); i++)
        vtk_density->SetTuple(i, density[i].data());
    
    mesh->GetPointData()->AddArray(vtk_density);
}