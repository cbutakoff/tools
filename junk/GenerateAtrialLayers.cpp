...
void usage(char *exe)
{
	...
	std::cout<<"-voxel_mc <float> \t - Voxel size for marching cubes(isotropic, default 0.5)"<<std::endl;
...
int main(int argc, char **argv)
{
	...
	for(int c=1; c<argc; c++)
	{
		...
		else if ( strcmp(argv[c],"-voxel_mc")==0 )
		{
			voxel_mc = atof(argv[++c]);
		}
...
//		}
	...
	std::cout<<"Resampling the masks: "<<vx/voxel_mc<<" "<<vy/voxel_mc
	<<" "<<vz/voxel_mc<<std::endl;
	{
		vtkSmartPointer<vtkImageResample> resamp = vtkSmartPointer<vtkImageResample>::New();
		resamp->SetInputData( mask_endo );
		resamp->SetAxisMagnificationFactor(0,vx/voxel_mc);
		resamp->SetAxisMagnificationFactor(1,vy/voxel_mc);
		resamp->SetAxisMagnificationFactor(2,vz/voxel_mc);
		resamp->SetAxisOutputSpacing(0, voxel_mc);
		resamp->SetAxisOutputSpacing(1, voxel_mc);
		resamp->SetAxisOutputSpacing(2, voxel_mc);
	...
	vtkSmartPointer<vtkPolyData> endo_mesh = vtkSmartPointer<vtkPolyData>::New();
	{
		...
		marchingcubes->SetValue(0,voxel_mc);
	...
	GenerateLayersAlongField(layers, epi_mesh, endo_mesh, nLayers, FIELD_LAPLACE, voxel_lap, laplace_iter);
