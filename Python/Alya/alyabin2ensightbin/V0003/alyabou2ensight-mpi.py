...
def read_header_alyabin(file_object):
    ...
    for i in range(0, 9):
        strings = strings + [
            read_one_fp90_record(file_object, 8, np.uint8).tobytes().decode().strip()
        ]
...
def write_geometry(number_of_blocks):
    ...
    connectivity = read_alya_array(
        os.path.join(inputfolder, f"{project_name}-LNODB{file_suffix}"),
        number_of_blocks,
    )

    # elements have ids local to each block, tranform them to global ids
    # a = connectivity['tuples_per_block'][0]
    a = partitions["Boundaries"][0]
    npts = partitions["Points"][0]
    for i in range(1, partitions["Boundaries"].shape[0]):  # for each block, skip 0
        b = a + partitions["Boundaries"][i]
...
def write_variable_perboun(varname, iteration, number_of_blocks):
    try:
        data = read_alya_variable(varname, iteration, number_of_blocks)
        if data["header"]["Association"] != ASSOCIATION.BOUNDARIES:
            return {
                "time_real": -1,
                "time_int": -1,
    ...
    try:
        with open(filename, "wb") as f:
            ...
            if data["header"]["VariableType"] == "scalar":
                ...
                data2write = np.zeros(
                    [inverse_pt_correspondence.max() + 1, 3], dtype=ensight_float_type
                )
...
if my_rank == 0:
    ...
    assert (
        pt_ids > 0
    ).all(), "Some points in the mesh do not have a correspondence in the parittions"
    pt_ids = None  # free memeory

    LEINV = read_alya_array(
        os.path.join(inputfolder, f"{project_name}-LBINV{file_suffix}"),
        number_of_blocks,
    )
    ...
    LTYPE = read_alya_array(
        os.path.join(inputfolder, f"{project_name}-LTYPB{file_suffix}"),
        number_of_blocks,
    )
...
if my_rank == 0:
    ...
    with open(os.path.join(outputfolder, case_file), "w") as f:
        ...
        timeline_id = 1
        timelines = {}
        # each timeline will be identified only by the number of elements
        c = 0
        for varname in variables:
            ...
            variable_info_per_variable = variable_info_per_variable + [
                {
                    "varname": varname,
                    "data": variable_info[variable_info.field == varname].sort_values(
                        by="iteration"
                    ),
        ...
        for var_data in variable_info_per_variable:
            ...
            if df.association != "FAILED":
                line = (
                    f"{df.variabletype} per element: {timeline_id} {varname} {project_name}.ensi.{varname}-"
                    + "*" * iterationid_number_of_digits
                    + "\n"
                )
                f.write(line)
