...
def write_geometry(number_of_blocks):
    ...
    connectivity = read_alya_array(os.path.join(inputfolder,f'{project_name}-LNODB{file_suffix}'),    \
                                   number_of_blocks)
...
if my_rank == 0:    
    ...
    LNINV = read_alya_array(os.path.join(inputfolder,f'{project_name}-LNINV{file_suffix}'), number_of_blocks)
...
def do_work(work):
    ...
    if work['filetype'] == 'geometry':
        ...
        info = write_variable_perboun(work['name'], work['iteration'], work['number_of_blocks'])
