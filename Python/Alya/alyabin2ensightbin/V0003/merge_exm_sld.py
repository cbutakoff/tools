...
if my_rank==0:
    ...
    parser.add_argument("--case", action='append', help="Path and case file", required=True)
...
iterationid_number_of_digits = 6 #numnber of digits in ensi variable names used for number of iteration
...
def calculate_interpolation_weights( mesh, mesh_coarse ):
    ...
    #for i in range(npts):
        ...
        subId  = vtk.mutable(0)
        dist2  = vtk.mutable(0)
        loc.FindClosestPoint( mesh_coarse.GetPoint(i), closest_point, cellId, subId, dist2 )
...
def write_geometry(surf_mesh : vtk.vtkPolyData, path : str, casename : str) -> None:
    ...
    connectivity = np.reshape( vtk_to_numpy( surf_mesh.GetPolys().GetData() ), (-1,4))  #N x 4, columnn 0 == 3, i.e. 3 points, columns 1:4 are point ids 
    assert all(connectivity[:,0]==3), 'The coarse mesh has elements that are not triangles'
...
def write_ensi_case( df, timesets ):
    ...
    with open( case_file, 'w') as f:
        ...
        timeset_df = pd.DataFrame( variable_tuples, columns=["Variable","Timeset"] ).sort_values(by="Timeset", ascending=True, axis=0)
        

        for varname, timeset in timesets.items():
            ndim = df[ df['Variable']==varname ]['ndim'].to_numpy()[0]
            vartype = 'scalar' if ndim==1 else 'vector'            
            f.write ( f'{vartype} per node: {timeset} {varname} {out_casename}.ensi.{varname}-'+                '*'*iterationid_number_of_digits+'\n'  )
        ...
        for timeset in timeset_df['Timeset'].unique():
            varname = timeset_df[ timeset_df['Timeset']==timeset ]['Variable'].to_numpy()[0] #get one variable with the timeset
            
            var_df = df[ df['Variable']==varname ].sort_values( by='Time', ascending=True, axis=0 )
...
def read_variable_pernode(filename) -> (float, np.ndarray):
    ...
    with open( filename,'rb') as f:
        ...
        assert time_part[:2]   == b't='          , f'File {filename} has no time value in the description line' 
        assert type_part[:5]   == b'type='       , f'File {filename} has no type in the description line' 
        assert part_word       == b'part'        , f'File {filename} is not a valid ensi variable' 
        assert coord_keyword   == b'coordinates' , f'File {filename} is not a valid ensi variable' 
...
#   Other functions
#
def save_geometry_and_get_iterp_weights( casename : str, coarse_mesh_vtp_filename : str, out_casename : str = None ):
    """ 
    Arguments:
        casename : str
    ...
    assert os.path.isfile(coarse_mesh_vtp_filename), f'Missing {coarse_mesh_vtp_filename}'
    ...
    interp_weights, interp_ptids = calculate_interpolation_weights( rdr.GetOutput().GetBlock(0), rdr_coarse.GetOutput() )

    if not out_casename is None:
        write_geometry( surf_mesh = rdr_coarse.GetOutput(), path = os.path.dirname(out_casename), casename = get_case_name(os.path.basename(out_casename)) )

    return {'weights': interp_weights, 'point_ids': interp_ptids, 'variables' : varnames } 



def process_one_variable( filenamein, filenameout, interp_weights, interp_ptids ) -> (float, int):
...
def process_filelist(files, interp_weights, interp_ptids):
    ...
    for i, filename in enumerate(files2scatter):
        ...
        if not filename is None:
            (prefix, varname, fileno) = re.match(  "^(.+)\.ensi\.(.+)-(\d+)$", filename ).groups()                                                                                       
            if not varname in variables_to_ignore:
                time, ndim = process_one_variable(filename, 
                                os.path.join( out_path, f"{out_casename}.ensi.{varname}-{fileno}" ), 
                                interp_weights, interp_ptids) 
                if (np.round(time, time_ndigits) >= time_interval[0]) and (np.round(time, time_ndigits) <= time_interval[1]):
                    results.append( (varname, int(fileno), np.round(time, time_ndigits), ndim ) )
...
def variables_equal_timesets( df_sorted, var1, var2 ):
    ...
    if var1_df.shape[0] == var2_df.shape[0]: #definitely not the same timeset
        timediff    = ( np.abs( var1_df['Time'].to_numpy() - var2_df['Time'].to_numpy() ) < time_check_tolerance ).all()
        fileno_diff = ( np.abs( var1_df['FileNo'].to_numpy() - var2_df['FileNo'].to_numpy() ) == 0 ).all() 
...
for casename in casenames:
    ...
    if my_rank==0:
        ...
        data4saving = save_geometry_and_get_iterp_weights( casename, coarse_mesh, os.path.join(out_path, f"{out_casename}.ensi.case") )
        ...
        for varname in data4saving['variables']:
            ...
            files.extend( glob.glob(os.path.join(path, f"{file_prefix}.ensi.{varname}-*") ) )
...
if my_rank==0:
    ...
    results_df = pd.DataFrame( itertools.chain(*results), columns=['Variable','FileNo','Time','ndim'] ) 
