...
try:
    if my_rank == 0:
        ...
        parser.add_argument("-f", "--format", help='Format of the data to expect: mpio, alyabin, auto(default)',
                            default='auto')
        parser.add_argument("-v", "--vtu", action='store_true', required=False,
                            help='Generate VTU with double precision of coordinates?')
        parser.add_argument("-m", "--multiples", help='Postprocess only timesteps that are multiples of this value',
                            default=1, type=int)
        parser.add_argument("-i", "--interval", help='Postprocess only timesteps in the specified time interval',
                            type=float, nargs=2, metavar=('time_start', 'time_end'))
        ...
        if args.format == 'alyabin':
            ...
            assert (mpio_file_present == True) | (
                        alyabin_file_present == True), f"Nothing to postprocess, make sure problem name is {project_name}"
...
def read_header_mpio(f):
    ...
    header = {'DataType': dt, 'Lines': lines, 'Columns': columns, 'TimeStepNo': timestep_no, 'Time': time,
...
def read_alya_variable(variable_name, iteration, number_of_blocks):
    if iteration >= 0:
        field_filename = os.path.join(inputfolder,
...
if my_rank == 0:
    if not time_interval is None:
        variable_info = variable_info[
            (variable_info["time_real"] >= time_interval[0]) & (variable_info["time_real"] <= time_interval[1])]
    ...
    with open(os.path.join(outputfolder, case_file), 'w') as f:
        ...
        for varname in variables:
            ...
            var_data = variable_info[variable_info.field == varname].sort_values(by='iteration')
            ...
            if not found:
                timelines.append( {"filenumbers":var_data.time_int.to_numpy(),
        ...
        for timeline_index, tl in enumerate(timelines):
            if tl["filenumbers"][0]>-1:
                ...
                f.write("\n".join(textwrap.wrap(filenumbers_string, width=70,break_long_words=False,break_on_hyphens=False)) + '\n')
                f.write('time values:\n')
                times_string = " ".join(f"{x:.8E}" for x in tl["times"])
                f.write("\n".join(textwrap.wrap(times_string, width=70,break_long_words=False,break_on_hyphens=False)) + '\n')
