...
try:
    ...
    print(
        "Failed to load VTK, will not save VTK with double precision for point coordinates"
    )
    print(
        "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    )
...
def alya2ensight(args_raw: list[str]):
    ...
    try:
        if my_rank == 0:
            ...
            parser.add_argument(
                "output_folder", help="Folder for the output ensight case"
            )
            parser.add_argument(
                "-f",
                "--format",
                help="Format of the data to expect: mpio, alyabin, auto(default)",
                default="auto",
            )
            parser.add_argument(
                "-v",
                ...
                action="store_true",
                required=False,
                help="Generate VTU with double precision of coordinates?",
            )
            parser.add_argument(
                "-m",
                "--multiples",
                help="Postprocess only timesteps that are multiples of this value",
                default=1,
                type=int,
            )
            parser.add_argument(
            ...
            if args.format == "alyabin":
                ...
                filename_mpio = os.path.join(
                    inputfolder, f"{project_name}-LNODS.post.mpio.bin"
                )
                filename_alyabin = os.path.join(
                    inputfolder, f"{project_name}-LNODS.post.alyabin"
                )
                mpio_file_present = os.path.isfile(filename_mpio)
                alyabin_file_present = os.path.isfile(filename_alyabin)
                assert (mpio_file_present == True) | (
                    alyabin_file_present == True
                ), f"Nothing to postprocess, make sure problem name is {project_name}"
                assert (
                    mpio_file_present != alyabin_file_present
                ), "Found both alyabin and mpio files. Specify format to use in the --format argument"
    ...
    iterationid_number_of_digits = (
        6  # how many digits to use for the iteration id in variable files
    )
    ...
    def read_header_mpio(f):
        ...
        if "ELEM" in association:
            ...
            raise Exception(
                f"Unsupported association: {association}. Only variables per node and per element are supported"
            )
    ...
    def read_header_alyabin(file_object):
        ...
        assert hasattr(
            file_object, "read"
        ), "read_header: argument is not a file object"
        ...
        for i in range(0, 9):
            strings = strings + [
                read_one_fp90_record(file_object, 8, np.uint8)
                .tobytes()
                .decode()
                .strip()
            ]
        ...
        if strings[4] == "NPOIN":
            ...
            raise Exception(
                f"Unsupported association: {strings[4]}. Only variables per node and per element are supported"
            )
        ...
        if strings[6] == "4BYTE":
            ...
            assert False, f"Alya id type {strings[6]} is not supported"
        ...
        header = {
            ...
            "Lines": number_of_tuples_total,
            "Columns": number_of_dimensions,
            "TimeStepNo": time_instant_int,
            "Time": time_instant_real,
    ...
    def read_alyampio_array(filename, number_of_blocks):
        with open(filename, "rb") as f:
            ...
            tuples = np.reshape(
                np.fromfile(f, dtype=np.dtype(header["DataType"])),
    ...
    def read_alyabin_array(filename, number_of_blocks):
        with open(filename, "rb") as f:
            ...
            for i in range(number_of_blocks):
                number_of_tuples_in_block = read_one_fp90_record(f, 1, alya_id_type)[
                    0
                ]  # stored by alya
                ...
                tuples[c : c + number_of_tuples_in_block, :] = np.reshape(
                    tuples_temp, (number_of_tuples_in_block, number_of_dimensions)
                )
    ...
    def read_alya_variable(variable_name, iteration, number_of_blocks):
        if iteration >= 0:
            field_filename = os.path.join(
                inputfolder,
                "%s-%s-%08d%s" % (project_name, variable_name, iteration, file_suffix),
            )
        else:
            field_filename = os.path.join(
                inputfolder, "%s-%s%s" % (project_name, variable_name, file_suffix)
            )
    ...
    def write_geometry_vtk_double(point_coordinates, elements, element_types):
        ...
        element_alya2vtk = {
            ...
            30: {"vtkid": vtk.VTK_TETRA, "Vertices": 4},
            32: {"vtkid": vtk.VTK_PYRAMID, "Vertices": 5},
            34: {"vtkid": vtk.VTK_WEDGE, "Vertices": 6},
            10: {"vtkid": vtk.VTK_TRIANGLE, "Vertices": 3},
            12: {"vtkid": vtk.VTK_QUAD, "Vertices": 4},
        }
    ...
    def write_geometry(number_of_blocks):
        point_coordinates = read_alya_array(
            os.path.join(inputfolder, f"{project_name}-COORD{file_suffix}"),
            number_of_blocks,
        )
        # Read connectivity (indices inside start with 1)
        connectivity = read_alya_array(
            os.path.join(inputfolder, f"{project_name}-LNODS{file_suffix}"),
            number_of_blocks,
        )
        ...
        point_coordinates2 = np.zeros(
            (inverse_pt_correspondence.max() + 1, ndim), dtype=ensight_float_type
        )
        ...
        if vtk_installed:
            write_geometry_vtk_double(
                point_coordinates, connectivity, element_types.ravel()
            )
        ...
        with open(os.path.join(outputfolder, f"{project_name}.ensi.geo"), "wb") as f:
            ...
            while iii < ndim:
                f.write(
                    point_coordinates[:, iii].ravel().astype(ensight_float_type)
                )  # x coord
                iii = iii + 1
            ...
            while iii < 3:
                f.write(
                    0 * point_coordinates[:, 0].ravel().astype(ensight_float_type)
                )  # x coord
                iii = iii + 1
            ...
            for elem_alya_id, elem_ensi_id in element_alya2ensi.items():
                ...
                element_locations = np.where(element_types == elem_alya_id)[
                    0
                ]  # returns 2 sets, take first
    ...
    def write_variable(varname, iteration, number_of_blocks):
        ...
        try:
            ...
            return {
                "time_real": -1,
                "time_int": -1,
                "variable_type": "FAILED",
                "variable_association": "FAILED",
            }
    ...
    def write_variable_percell(data, varname, iteration):
        ...
        with open(
            os.path.join(outputfolder, fmt % (project_name, varname, iteration)), "wb"
        ) as f:
            f.write(b"description line 1".ljust(80))
            f.write(b"part".ljust(80))
            f.write(np.array([1], dtype=ensight_id_type))  # int
        ...
        return {
            "time_real": data["header"]["Time"],
            "time_int": data["header"]["TimeStepNo"],
            "variable_type": data["header"]["VariableType"],
            "variable_association": data["header"]["Association"],
        }
    ...
    def write_variable_pernode(data, varname, iteration):
        ...
        if iteration >= 0:
            filename = os.path.join(
                outputfolder, fmt % (project_name, varname, iteration)
            )
        else:
        ...
        try:
            with open(filename, "wb") as f:
                f.write(
                    bytes(
                        f"t={data['header']['Time']},type={data['header']['VariableType']}".ljust(
                            80
                        ),
                        "utf-8",
                    )
                ...
                if data["header"]["VariableType"] == "scalar":
                    data2write = np.zeros(
                        inverse_pt_correspondence.max() + 1, dtype=ensight_float_type
                    )
                    ...
                    data2write = np.zeros(
                        [inverse_pt_correspondence.max() + 1, 3],
                        dtype=ensight_float_type,
                    )
                    ncomponents = data["tuples"].shape[1]  # for 1d, 2d and 3d problems
                    data2write[inverse_pt_correspondence, 0:ncomponents] = data[
                        "tuples"
                    ]
                    f.write(
                        data2write.ravel(order="F").astype(ensight_float_type)
                    )  # z coord
                else:
    ...
    if my_rank == 0:
        ...
        assert (
            npartitions == partitions.shape[0]
        ), f"Error reading partitions .alyapar file, read {partitions.shape[0]} instead of {npartitions}"

        number_of_blocks = partitions.shape[0]

        partitions = pandas.DataFrame(
            partitions[:, :4], columns=["id", "Elements", "Points", "Boundaries"]
        )
        ...
        variable_info = pandas.DataFrame(
            {
                ...
                "filename": new_field_filelist,
            }
        )
        variable_info["time_int"] = 0
        variable_info["time_real"] = 0.0
    ...
    if my_rank == 0:
        ...
        LNINV = read_alya_array(
            os.path.join(inputfolder, f"{project_name}-LNINV{file_suffix}"),
            number_of_blocks,
        )
        inverse_pt_correspondence = (LNINV["tuples"] - 1).ravel()
        # convert ids to python
        ...
        assert (
            ...
            "Some points in the mesh do not have a correspondence in the parittions"
        )
        pt_ids = None  # free memeory

        LEINV = read_alya_array(
            os.path.join(inputfolder, f"{project_name}-LEINV{file_suffix}"),
            number_of_blocks,
        )
        inverse_el_correspondence = (LEINV["tuples"] - 1).ravel()
        # convert ids to python
        inverse_el_correspondence = np.argsort(
            inverse_el_correspondence
        )  # i'm still a bit puzzled, but it works

        LTYPE = read_alya_array(
            os.path.join(inputfolder, f"{project_name}-LTYPE{file_suffix}"),
            number_of_blocks,
        )
    ...
    def CreateWork(variable_info, number_of_blocks):
        q = []
        q.append(
            json.dumps({"filetype": "geometry", "number_of_blocks": number_of_blocks})
        )
    ...
    def do_work(work):
        ...
        if work["filetype"] == "geometry":
            ...
            info = write_variable(
                work["name"], work["iteration"], work["number_of_blocks"]
            )
    ...
    def process_result(result):
        if result["filetype"] == "variable":
            ...
            variable_info.loc[index, "time_real"] = float(result["time_real"])
            variable_info.loc[index, "time_int"] = int(result["time_int"])
    ...
    if my_rank == 0:
        if not time_interval is None:
            variable_info = variable_info[
                (variable_info["time_real"] >= time_interval[0])
        ...
        with open(os.path.join(outputfolder, case_file), "w") as f:
            ...
            # put the timelines separately to easily merge them in a dict:
            # timelines[i] = {"filenumbers":List[int], "times":List[float]}
            timelines = []
            ...
            for varname in variables:
                ...
                var_data = variable_info[variable_info.field == varname].sort_values(
                    by="iteration"
                )

                # check if timelines already has this timeline
                found = False
                for idx, tl in enumerate(timelines):
                    if np.array_equal(var_data.time_int.to_numpy(), tl["filenumbers"]):
                        timeline_id = idx + 1
                        found = True
                        break
                ...
                if not found:
                    timelines.append(
                        {
                            "filenumbers": var_data.time_int.to_numpy(),
                            "times": var_data.time_real.to_numpy(),
                        }
                    )
                    timeline_id = len(timelines)

                variable_info_per_variable = variable_info_per_variable + [
                    {"varname": varname, "data": var_data, "timeline": timeline_id}
                ]
            ...
            for var_data in variable_info_per_variable:
                ...
                if df.association != "FAILED":
                    ...
                    if df.association == ASSOCIATION.NODES:
                        ...
                        assert (
                            False
                        ), "Only per element and per node variables are supported"

                    line = (
                        f"{df.variabletype} per {ensi_association}: {timeline_id} {varname} {project_name}.ensi.{varname}-"
                        + "*" * iterationid_number_of_digits
                        + "\n"
                    )
                    f.write(line)
            ...
            f.write("\n")
            f.write("TIME\n")
            for timeline_index, tl in enumerate(timelines):
                if tl["filenumbers"][0] > -1:
                    f.write(f"time set: {timeline_index+1}\n")
                    number_of_timesteps = tl["filenumbers"].shape[0]
                    f.write(f"number of steps: {number_of_timesteps}\n")
                    f.write(f"filename numbers: \n")
                    filenumbers_string = " ".join(str(x) for x in tl["filenumbers"])
                    f.write(
                        "\n".join(
                            textwrap.wrap(
                                filenumbers_string,
                    ...
                    f.write("time values:\n")
                    times_string = " ".join(f"{x:.8E}" for x in tl["times"])
                    f.write(
                        "\n".join(
                            textwrap.wrap(
                                times_string,
