...
def get_DISPL_filenumbers(casename) -> int:
    ...
    return (
        np.array([int(x) for x in filename_numbers], dtype=int),
        np.array([float(x) for x in time_values]),
        displ_timeset,
    )


def update_casefile_newvariables(
    casefilename, vars, displ_filename_numbers, displ_timeset_no
):
    ...
    for varname, vardata in vars.items():
        ...
        if vardata["ntimeset"] != displ_timeset_no:
            ...
            line.append(
                " ".join(
                    [
                        str(x) + "\n" if i % 12 == 0 else str(x)
                        for i, x in enumerate(vardata["file_numbers"])
                    ]
                )
            )
            ...
            line.append(
                " ".join(
                    [
                        str(x) + "\n" if i % 12 == 0 else str(x)
                        for i, x in enumerate(vardata["time_values"])
                    ]
                )
            )
...
for filename in glob.glob(
    os.path.join(
        os.path.dirname(casename_sld),
        f"{get_case_name(os.path.basename(casename_sld))}*",
    ),
    recursive=False,
):
...
filename_numbers, time_values_tmp, displ_timeset_no = get_DISPL_filenumbers(
    casefile_out
)
...
if solidz_times.max() != time_values_tmp.max():
    print(
        "Max time in solidz ({solidz_times.max()}) and exmedi ({time_values_tmp.max()}) are different"
    )
...
interp_weights, interp_ptids = calculate_interpolation_weights(
    rdr_exm.GetOutput().GetBlock(0), rdr_sld.GetOutput().GetBlock(0)
)
...
for ivar, varname in enumerate(varnames):
    ...
    written_vars[varname]["ntimeset"] = (
        ivar + rdr_sld.GetTimeSets().GetNumberOfItems() + 1
    )
...
for ti, t in enumerate(solidz_times):
    ...
    for varname in varnames:
        ...
        if exm_array is None:
            ...
            arr = interpolate_array(
                vtk_to_numpy(mesh_exm.GetPointData().GetArray(varname)),
                interp_weights,
                interp_ptids,
            )
            write_variable_pernode(
                arr,
                out_path,
                get_case_name(os.path.basename(casename_sld)),
                varname,
                filename_numbers[ti],
            )
...
update_casefile_newvariables(
    casefile_out, written_vars, filename_numbers, displ_timeset_no
)
