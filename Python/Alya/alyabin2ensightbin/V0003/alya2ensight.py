...
def write_geometry(number_of_blocks):
    ...
    if vtk_installed:
        write_geometry_vtk_double(
            point_coordinates, connectivity, element_types["tuples"].ravel()
        )
    ...
    with open(os.path.join(outputfolder, f"{project_name}.ensi.geo"), "wb") as f:
        ...
        for elem_alya_id, elem_ensi_id in element_alya2ensi.items():
            ...
            element_locations = np.where(element_types["tuples"] == elem_alya_id)[
                0
...
LNINV = read_alya_array(
    os.path.join(inputfolder, f"{project_name}-LNINV{file_suffix}"), number_of_blocks
)
...
LTYPE = read_alya_array(
    os.path.join(inputfolder, f"{project_name}-LTYPE{file_suffix}"), number_of_blocks
)
...
def do_work(work):
    ...
    if work["filetype"] == "geometry":
        ...
        if "MATER" in work["name"]:
            info = write_variable_percell(
                work["name"], work["iteration"], work["number_of_blocks"]
            )
        else:
            info = write_variable_pernode(
                work["name"], work["iteration"], work["number_of_blocks"]
            )
        ...
        with open(os.path.join(outputfolder, case_file), "w") as f:
            ...
            for var_data in variable_info_per_variable:
                ...
                if df.association != "FAILED":
                    line = (
                        f"{df.variabletype} per {df.association}: {timeline_id} {varname} {project_name}.ensi.{varname}-"
                        + "*" * iterationid_number_of_digits
                        + "\n"
                    )
                    f.write(line)
