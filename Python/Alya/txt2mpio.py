...
def write_vector_mpio(filename, vector, header):
    ...
    with open(filename, 'wb') as f:
        ...
        vector[:,1:].astype(header['DataTypePython']).tofile(f)
...
#read header
header = {}
with open(filenamein+'.hdr','r') as ff:
    for line in ff:
        dd = line.split(":")
        header[dd[0].strip()] = dd[1].strip()

v = np.loadtxt(filenamein+'.csv')


write_vector_mpio( filenameout, v, header )
